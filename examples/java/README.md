...
The same programs are ported to other JVM languages in
[`../clojure`](../clojure) and [`../scala`](../scala).

## Running an example

### With the downloaded release jars

From the repository root, which holds the two jars (the main jar and the
`natives-<platform>` jar for your OS — `linux-x86_64`, `osx-aarch_64`, or
`windows-x86_64`):

```bash
java --enable-native-access=ALL-UNNAMED \
  -cp jopendaq-0.1.0.jar:jopendaq-0.1.0-natives-linux-x86_64.jar \
  examples/java/StreamReaderExample.java
```
