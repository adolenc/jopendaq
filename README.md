...
```clojure
(import '(org.opendaq Instance Channel StreamReader))
...
the GitHub release pinned in `src/main/resources/org/opendaq/native-binaries.properties`,
...
- `org.opendaq.lowlevel` contains low-level bindings that closely mirror the
...
- `org.opendaq` contains the high-level API: one wrapper class per openDAQ
